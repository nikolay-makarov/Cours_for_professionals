...
with open('food_services.json', 'r', encoding='utf-8') as file:
    ...
    company = {}
    for elem in data:
        district[elem['District']] = district.get(elem['District'], 0) + 1
        if elem['OperatingCompany']:
            company[elem['OperatingCompany']] = company.get(elem['OperatingCompany'], 0) + 1
print(*max(district.items(), key=lambda x: x[1]), sep=': ')
