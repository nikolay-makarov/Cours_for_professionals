...
Также ему нравится плавать по длинным дорожкам,
поэтому из всех работающих в это время бассейнов нужно выбрать бассейн с наибольшей длиной дорожки,
...
for pool in sorted_pools:
    if t := pool['WorkingHoursSummer'].get('Понедельник'):
        if (datetime.strptime(t.split('-')[0], '%H:%M').time() <= time(10, 0) and
                datetime.strptime(t.split('-')[1], '%H:%M').time() >= time(12, 0)):
