...
Оказалось, что некий молодежный клуб решил подшутить, распространяя всякие глупости. Однако подобное хулиганство мешает
...
category_news = list(filter(lambda x: x[1] == data[-1][0], data[:-1]))
category_news.sort(key=lambda x: (float(x[2]), x[0][0]))
[print(row[0]) for row in category_news]
